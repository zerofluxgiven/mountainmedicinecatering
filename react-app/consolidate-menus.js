 ...
 * 2. Move any menus from 'menus' to 'menu_items' if they don't exist there
...
async function consolidateMenus() {
  ...
  if (!db) {
    console.error('❌ Firebase not found. Make sure you\'re logged into the app');
    return;
  }
  ...
  try {
    ...
    for (const menuDoc of menusSnap.docs) {
      ...
      if (menuItemsMap.has(key)) {
        ...
        console.log(`   ⚠️  Duplicate found in menu_items`);
      ...
      deleted++;
      console.log(`   🗑️  Deleted from menus collection`);
    }
    
    console.log(`\n✅ Consolidation complete!`);
    console.log(`   📦 Moved: ${moved} menus`);
    console.log(`   🗑️  Deleted: ${deleted} menus`);
