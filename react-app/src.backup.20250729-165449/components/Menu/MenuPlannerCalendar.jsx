...
export default function MenuPlannerCalendar({ eventId, menuId, onMenuChange, standaloneConfig }) {
  ...
  const saveMenuToDatabase = useCallback(async (menuToSave) => {
    ...
    try {
      ...
      // Remove the id field from menuData to avoid Firestore errors
      delete menuData.id;

      // Check if menu has an ID (either from props or from previous save)
      const effectiveMenuId = menuToSave.id || menuId;
      
      if (effectiveMenuId && effectiveMenuId !== 'new') {
        // Update existing menu
        await updateDoc(doc(db, 'menus', effectiveMenuId), menuData);
      } else {
        // Create new menu only if it doesn't have an ID
