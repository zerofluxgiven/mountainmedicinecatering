...
def file_manager_ui(user):
    ...
    st.markdown('### 🔍 Search Uploads')
    search_term = st.text_input('Enter filename or tag...')

    if 'view_mode' not in st.session_state:
        st.session_state['view_mode'] = 'Compact'
    view_mode = st.radio('View Mode', ['Compact', 'Expanded'], horizontal=True, index=['Compact', 'Expanded'].index(st.session_state['view_mode']))

    if st.button('Edit Metadata'):
        st.session_state['show_metadata_editor'] = not st.session_state.get('show_metadata_editor', False)
